...
class MysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, TXContentItem):
            # 如果存在，则不做处理
            if not self.checkDetailExist(cursor, item['title']):
                spider.logDao.info(u'存腾讯详情：' + item['title'])
                sql = "insert into tengxun_detail (channel_name, tags, post_date,post_user,page_content,title,source_url," \
                ...
                cursor.execute(sql, (item['channel_name'],
                                     ...
                                     update_time))
                spider.logDao.info(u'存腾讯详情：' + item['title'] + u'  成功')
            else:
                spider.logDao.info(u'存腾讯详情已经存在：' + item['title'])
        else:
            pass
        cursor.close()
    ...
    def checkDetailExist(self, cursor,  title):
        sql_query = 'select title from tengxun_detail where title=%s'
