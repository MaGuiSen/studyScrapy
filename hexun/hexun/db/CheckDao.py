...
class CheckDao(object):
    ...
    def checkExist(self, source_url):
        ...
        if not cursor:
            return True
        sql_query = 'select id from hexun_detail where hash_code=%s'
    ...
    def getHtml(self, pageIndex):
        ...
        if not cursor:
            return []
        sql_query = 'select id,content_html from hexun_detail group by id limit %s, %s'
    ...
    def getPostTime(self, pageIndex):
        ...
        if not cursor:
            return []
        sql_query = 'select id,post_date from hexun_detail group by id limit %s, %s'
        cursor.execute(sql_query, ((pageIndex - 1) * 15, 15))
        results = cursor.fetchall()
        cursor.close()
        return results or []
    ...
    def updatePostTime(self, id, post_date):
        ...
        if not cursor:
            return
        sql_query = "update hexun_detail set post_date=%s where id=%s"
    ...
    def updateStyles(self, id, styles):
        ...
        if not cursor:
            return
        sql_query = "update hexun_detail set styles=%s where id=%s"
    ...
    def updateHtml(self, id, content_html):
        ...
        if not cursor:
            return
        sql_query = "update hexun_detail set content_html=%s where id=%s"
