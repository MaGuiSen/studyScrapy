...
class MysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, ContentItem):
            # 如果存在，则不做处理
            spider.logDao.info(u'存界面详情：' + item['title'])
            sql = "insert into jiemian_detail (" \
            ...
            try:
                cursor.execute(sql, (
                    ...
                    item['src_ref'],
                    update_time))
                spider.logDao.info(u'存界面详情：' + item['title'] + u'  成功' + u' ' + item['post_date'])
            except Exception as e:
                spider.logDao.warn(str(e))
                spider.logDao.warn(u'存界面详情：' + item['title'] + u'  失败')
        else:
            pass
        cursor.close()
